...
export class DatabaseService {
  ...
  async addPayment(payment: any): Promise<void> {
    if (!this.db) throw new Error('数据库未初始化');
    await this.db.execute(
      `INSERT INTO payments (project_id, payment_amount, payment_date, payment_type, note)
       VALUES ($1, $2, $3, $4, $5)`,
      [payment.project_id, payment.payment_amount, payment.payment_date, payment.payment_type, payment.note]
    );
  }
   ...
   * 添加分包商
   */
  async addSubcontractor(sub: any): Promise<void> {
    if (!this.db) throw new Error('数据库未初始化');
    await this.db.execute(
      `INSERT INTO subcontractors (project_id, name, contract_amount, paid_amount, contact_person, phone)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [sub.project_id, sub.name, sub.contract_amount, sub.paid_amount, sub.contact_person, sub.phone]
    );
  }
  ...
  async getSchedules(projectId: number): Promise<any[]> {
    if (!this.db) throw new Error('数据库未初始化');
    return await this.db.select<any[]>(
      'SELECT * FROM schedules WHERE project_id = $1 ORDER BY planned_start_date',
      [projectId]
    );
  }
  ...
  async addSchedule(schedule: any): Promise<void> {
    if (!this.db) throw new Error('数据库未初始化');
    await this.db.execute(
      `INSERT INTO schedules (project_id, planned_start_date, planned_end_date, actual_start_date, actual_end_date, progress_percent, milestone_name)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [schedule.project_id, schedule.planned_start_date, schedule.planned_end_date,
       schedule.actual_start_date, schedule.actual_end_date, schedule.progress_percent, schedule.milestone_name]
    );
  }
   ...
   * 比较最新记录的更新时间
   */
  async checkForUpdates(): Promise<{ hasUpdate: boolean; lastUpdate: string | null }> {
    if (!this.db) throw new Error('数据库未初始化');
    const result = await this.db.select<any[]>(
      `SELECT MAX(updated_at) as last_update FROM (
        ...
        UNION ALL
        SELECT MAX(extracted_at) as updated_at FROM extracted_fields
        UNION ALL
        SELECT MAX(updated_at) as updated_at FROM projects
      )`
    );
    return {
      hasUpdate: result.length > 0 && result[0]?.last_update != null,
      lastUpdate: result[0]?.last_update || null,
    };
  }
  ...
  async clearAll(): Promise<void> {
    ...
    await this.db.execute('DELETE FROM subcontractors');
